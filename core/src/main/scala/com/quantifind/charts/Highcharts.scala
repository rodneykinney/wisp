...
object Highcharts extends BinnedDataLowerPriorityImplicits with HighchartsStyles {
  ...
  implicit class PairFromIterableFunction[A, B](ab: (Iterable[A], A => B)) extends
  IterablePair[A, B] {
    def toIterables: (Iterable[A], Iterable[B]) = (ab._1, ab._1.map(ab._2))
  }

  implicit class PairFromFunctionIterable[A, B](ab: (A => B, Iterable[A])) extends
  IterablePair[A, B] {
  ...
  implicit class PairFromArray[B](b: (Array[B])) extends IterablePair[Int, B] {
    def toIterables = ((0 until b.size), b)
  }
