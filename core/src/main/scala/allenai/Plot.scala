...
object Plot extends WebPlotter with IterablePairConversions with ContainerConversions {
  ...
  def line(data: LinePlotData) = {
    val hc = data match {
      case xy: XYData =>
        Highchart(xy.x, xy.y, SeriesType.line)
      case xy: LabeledXData =>
        val (x, labels) = (xy.x, xy.labels)
        Highchart(x, (0 until x.size), SeriesType.line)
